...
- [x] **Phase 3 — Ops realism** — core ops done 2026-07-12: payout tick (held→payable per hold-days, SQL fn + nightly VPS cron 02:40 UTC via ~/scripts/mytrails-ops.sh) + nightly JSON backups (~/backups/mytrails/, keep 7) + loading guards for async hydration. DEFERRED (need Joey's design call, not skipped silently): in-app notification center + email-outbox page (new UX surface — design-system rule says ask first), mobile audit + skeleton pass. vps-stats already monitors mytrails (up).
