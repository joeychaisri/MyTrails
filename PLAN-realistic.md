...
- [x] **Phase 3 — Ops realism** — core ops done 2026-07-12: payout tick (held→payable per hold-days, SQL fn + nightly VPS cron 02:40 UTC via ~/scripts/mytrails-ops.sh) + nightly JSON backups (~/backups/mytrails/, keep 7) + loading guards for async hydration. Joey approved 2026-07-12: notification center + email outbox BUILT (bell in dashboard header + /organizer/outbox, derived from store, works both modes). Scheduled publish now server-side via pg_cron. LIVE SITE FLIPPED TO SUPABASE. Still open: mobile audit + skeleton pass (minor).
